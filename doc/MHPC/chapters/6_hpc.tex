...
\begin{itemize}
  \item \textbf{Booster module (Atos BullSequana X2135 "Da Vinci" Blade)}:
    ...
    \begin{itemize}
      \item \textbf{Processor}: 1$\times$ Intel Xeon Platinum 8358
        (Ice Lake), 32
        cores at 2.60 GHz
      \item \textbf{Memory}: 512 GB DDR4 RAM
      \item \textbf{GPU Accelerators}: 4$\times$ NVIDIA Ampere A100
        (64 GB HBM2e)
    \end{itemize}
...
\begin{enumerate}
  \item \textbf{Data Acquisition}: Automated retrieval of seismic waveforms
    from data servers. To prevent server timeouts, downloads were
    segmented into
    days of interval.
  ...
  \item \textbf{Event Location}: Non-linear hypocenter determination using the
    NonLinLoc software package, employing probabilistic global
    search algorithms
...
\begin{itemize}
  ...
  \item \textbf{Configuration Management}: The pipeline uses Hydra
    ...
    configuration management. Hydra organizes all processing
    parameters, such as
    ...
    parameter sweeps (\textit{e.g.} varying detection thresholds
      across 0.1, 0.2,
  ...
  \item \textbf{Disk Space Optimization}: Symbolic links are used to share
    intermediate results between processing stages, reducing storage
    requirements
...
\begin{table}[h]
  ...
  \begin{tabular}{lccc}
    \hline
    \textbf{Pipeline Stage} & \textbf{Nodes} & \textbf{Tasks/Node} &
    \textbf{Threads/Task} \\
    \hline
