...
\begin{equation}
  S_t = \max\left(0, 1 - \frac{|t_{\text{pred}} -
  t_{\text{ref}}|}{\sigma_w}\right)
\end{equation}
...
\begin{table}[h!]
  ...
  \begin{tabular}{cc||c|c|c|}
    \hline
    \multirow{3}{*}{\rotatebox{90}{\textbf{Base}}} & P    & $MH_{P}$
    & $\epsilon_{PS}$ & $MS_P$       \\\cline{2-5}
  ...
  \begin{tabular}{cc||c|c|}
    \hline
    \multirow{2}{*}{\rotatebox{90}{\textbf{Base}}}  & Event &
    $MH_{E}$ & $MS_{E}$        \\\cline{2-4}
...
\begin{enumerate}
  ...
  \item \textbf{Temporal Proximity:} The detected pick time falls within a time
    window $\omega_t = 0.5$ seconds of the catalog pick time. This
    time interval
...
\begin{itemize}
  \item $\delta t$ represents the absolute arrival time difference;
    $$\delta t =
    \begin{cases}
      1 - \frac{|T_t - B_t|}{\omega_t}, & \text{if } |T_t - B_t|
      \leq \omega_t \\
      0, & \text{otherwise}
    \end{cases}$$
    where $T_t$ and $B_t$ are the detected and catalog pick times,
    respectively,
...
\begin{enumerate}
  \item \textbf{Temporal Proximity}: The origin time discrepancy between the
    ...
    seconds. This threshold accommodates uncertainties arising from
    phase picking
...
\begin{itemize}
  ...
  \item $\delta d$ is the epicentral distance between detected and catalog
    ...
    $$\delta d =
    \begin{cases}
      1 - \frac{||T_d - B_d||_2}{\omega_d},&\text{if
      }||T_d-B_d||_2\leq\omega_d\\
      0, & \text{otherwise}
    \end{cases}$$
...
\begin{itemize}
  \item \textbf{Recall}:
    $$\mathrm{Recall}_P = \dfrac{\mathrm{MH}_P}{\mathrm{MH}_P +
    \mathrm{MS}_P + \epsilon_{PS}}$$
    $$\mathrm{Recall}_S = \dfrac{\mathrm{MH}_S}{\mathrm{MH}_S +
    \mathrm{MS}_S + \epsilon_{SP}}$$
    $$\mathrm{Recall}_{\text{Picks}} = \dfrac{\mathrm{MH}_P +
    \mathrm{MH}_S}{\mathrm{MH}_P + \mathrm{MH}_S + \mathrm{MS}_P +
    \mathrm{MS}_S + \epsilon_{PS} + \epsilon_{SP}}$$
    $$\mathrm{Recall}_{\text{Events}} =
    \dfrac{\mathrm{MH}_E}{\mathrm{MH}_E + \mathrm{MS}_E}$$
  ...
  \item \textbf{False Detection Rate (\ac{FDR})}:
    $$\mathrm{\ac{FDR}}_P = \dfrac{\mathrm{PS}_P}{\mathrm{MH}_P +
    \mathrm{PS}_P + \epsilon_{SP}}$$
    $$\mathrm{\ac{FDR}}_S = \dfrac{\mathrm{PS}_S}{\mathrm{MH}_S +
    \mathrm{PS}_S + \epsilon_{PS}}$$
    $$\mathrm{\ac{FDR}}_{\text{Picks}} = \dfrac{\mathrm{PS}_P +
    \mathrm{PS}_S}{\mathrm{MH}_P + \mathrm{MH}_S + \mathrm{PS}_P +
    \mathrm{PS}_S + \epsilon_{PS} + \epsilon_{SP}}$$
    $$\mathrm{\ac{FDR}}_{\text{Events}} =
    \dfrac{\mathrm{PS}_E}{\mathrm{MH}_E + \mathrm{PS}_E}$$
