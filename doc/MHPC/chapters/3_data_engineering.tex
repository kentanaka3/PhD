...
\begin{enumerate}
  ...
  \item Applies channel priority filters (HH, EH, HN, HG, details in section
    \ref{subsec:ChannelPriorities}) to select the highest-quality
    available data.
...
\begin{table}[h]
  ...
  \begin{tabular}{llp{7cm}}
    \hline
    \textbf{Extension} & \textbf{Parser Class} & \textbf{Content
    Description} \\
    \hline
    ...
    .dat & \texttt{DataFileDAT} & Input phase picks (P and S wave
    arrival times) in
...
The \texttt{DataCatalog} class provides multi-format support and
catalog merging
capabilities, automatically dispatching to the appropriate
