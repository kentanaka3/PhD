% Purpose: High-Performance Computing deployment, workload
% scheduling, workflow orchestration, and parallel scaling on the
% CINECA Leonardo supercomputer.
...
Jobs are submitted to the production Booster partition
\texttt{boost\_usr\_prod} under defined QoS classes tailored to the
developmental lifecycle:
\begin{itemize}
  \item \textbf{Debug QoS (\texttt{boost\_qos\_dbg})}: Dedicated to
    rapid pipeline validation, integration smoke tests, and
    hyperparameter checks. Limited to a maximum allocation of 2
    nodes, 8 GPUs, and 30 minutes of wall-clock time.
  \item \textbf{Production QoS (\texttt{boost\_qos\_bprod})}:
    Allocated for large-scale multi-year retrospective
    re-evaluations. Supports long-running array jobs spanning dozens
    of nodes with wall-clock limits up to 24 hours.
\end{itemize}
