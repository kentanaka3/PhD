...
Section~\ref{sec:software_architecture} presents the software design patterns
and the central configuration module \texttt{ogsconstants}.
Section~\ref{sec:weight_conversion} establishes the formal
weight-to-uncertainty mapping. Section~\ref{sec:data_acquisition} describes the
distributed \ac{FDSN} continuous waveform acquisition architecture.
Section~\ref{sec:catalog_parsing} formulates the extensible multi-format
catalog parsing hierarchy. Section~\ref{sec:catalog_aggregation} details the
automated format detection, merging logic, and columnar persistence.
Section~\ref{sec:visualization_framework} presents the object-oriented
visualization framework and geodetic math. Finally,
Section~\ref{sec:storage_strategy} outlines the tiered storage strategy and
...
scientific computing libraries including NumPy, pandas, ObsPy (\cite{ObsPy}),
Pyrocko, Cartopy, and scikit-learn. To ensure maintainability, maintain loose
coupling between processing stages, and eliminate hardcoded parameters, the
system adheres to the \textbf{Single Source of Truth} design pattern.
  ...
  \item \textbf{Parallel Execution and HPC Hardware Allocation}:
    \begin{itemize}
      ...
      \item \texttt{GPU\_RANK}, \texttt{GPU\_SIZE}: Device identifiers managing
        CUDA device affinity for multi-GPU inference across NVIDIA A100 nodes.
  ...
  \item \textbf{Phase Identifiers and Detection Thresholds}:
    \begin{itemize}
      ...
      \item \texttt{THRESHOLDS}: Systematic confidence threshold array spanning
        $0.1$ to $0.9$ in increments of $0.1$, constructed via
        \texttt{np.linspace(0.1, 0.9, 9)}.
      \item \texttt{PWAVE\_THRESHOLD = SWAVE\_THRESHOLD = 0.1}: Default minimum
        model probability threshold for accepting \ac{ML} phase candidates.
    \end{itemize}
  ...
  \item \textbf{Spatial Domain and Polygon Boundaries}:
    \begin{itemize}
      \item \texttt{OGS\_POLY\_REGION}: An eight-vertex convex
        ...
        \begin{align}
          \mathcal{P}_{\mathrm{OGS}} = \{ &(10.0, 45.5),\, (10.0,
            46.5),\, (11.5, 47.0),\, (12.5, 47.0), \nonumber \\
...
formulation (\cite{Hypo71}) establishes a discrete integer weighting
scale $w \in \{0, 1, 2, 3, 4\}$. To utilize these picks in
probabilistic relocation algorithms (such as NonLinLoc) or in
...
Table~\ref{tab:h71_weights} defines the quantitative mapping
implemented in the pipeline. Picks assigned weight 0 represent
ideal, impulsive onsets with timing uncertainties below 10
milliseconds ($\sigma_t < 0.01\,\mathrm{s}$), whereas weight 4
denotes poor-quality picks with uncertainties exceeding one second
($\sigma_t \ge 1.0\,\mathrm{s}$).

\begin{table}[htbp]
  \centering
  \small
  \caption{Mapping between analyst-assigned Hypo71 pick weight
    codes, timing uncertainties ($\sigma_t$), and physical
    interpretation implemented in the \ac{OGS} pipeline. The reader should
    notice that weight codes represent an approximately logarithmic
    scaling of picking uncertainty. Weight codes and thresholds
    derived from official \ac{OGS} operational catalog conventions and
  anchored in \texttt{ogsconstants.H71\_OFFSET}.}
  ...
  \begin{threeparttable}
    \begin{tabularx}{\linewidth}{c c X}
      ...
      \midrule
      0 & $< 0.01$     & Impulsive onset,
      sharp first motion, high SNR \\
      1 & $0.01\text{--}0.04$ & Clear onset, unambiguous first break \\
      2 & $0.04\text{--}0.20$ & Fairly clear onset, moderate background
      noise \\
      3 & $0.20\text{--}1.00$ & Emergent
      onset, low SNR, gradual amplitude rise \\
      4 & $\ge 1.00$    & Poor quality pick, emergent coda, highly uncertain \\
      5 & $25.00$      & Very uncertain / rejected pick (rarely utilized) \\
      \bottomrule
    \end{tabularx}
...
\texttt{MassDownloader} (\cite{ObsPy}) and Pyrocko
\texttt{squirrel}—controlled through a unified command-line argument schema.
...
\begin{enumerate}
  ...
  \item \textbf{ETH Zurich}: Swiss Seismological Service (SED),
...
t_{\mathrm{end}}]$ into discrete \textbf{24-hour UTC calendar days}:
\begin{equation}
  \mathcal{D}_k = \left[ t_{\mathrm{start}} + k \cdot \Delta
    t_{\mathrm{day}},\, t_{\mathrm{start}} + (k + 1) \cdot \Delta
...
\begin{itemize}
  ...
  \item \textbf{Lock-Free Parallelism}: Multiple MPI ranks or SLURM
...
\begin{enumerate}
  \item \textbf{HH[ZNE]}: High-Gain Broadband Seismometer
    ($100\,\mathrm{Hz}$ sampling; primary choice for microseismic picking).
...
(Figure~\ref{fig:parser_architecture}). \texttt{OGSDataFile}
...
\begin{figure}[htbp]
  ...
  \begin{tikzpicture}[
    ...
    {\textbf{DataFileTXT}\\\scriptsize .txt ($M_{\mathrm{L}}$ magnitudes)};
    \node[parser, right=6mm of txt] (pun)
    {\textbf{DataFilePUN}\\\scriptsize .pun (Punch card)};
  ...
  \caption{Class inheritance and aggregation architecture of the \ac{OGS}
    ...
    The reader should notice the inheritance hierarchy rooted at
    \texttt{OGSCatalog}, specialized via \texttt{OGSDataFile}, and
  orchestrated by the \texttt{DataCatalog} aggregator.}
...
\end{verbatim}
  ...
  \begin{enumerate}
    \item \textbf{Pick Normalization and Concatenation}: Pick
      ...
      \texttt{normalize\_event\_groups()} to ensure consistent join
      keys across HPL, DAT, and PUN files.
    \item \textbf{Event Attribute Outer Joins}: Merging events from
      distinct files requires joining complementary fields:
      \begin{itemize}
        \item Primary hypocenters, origin times, depths, and spatial
          uncertainties are seeded from HPL files.
        \item TXT files contribute revised local magnitude
          ($M_{\mathrm{L}}$), duration magnitude ($M_{\mathrm{D}}$),
          and station count metrics, joined via an outer merge on
          event index \texttt{idx}.
        \item PUN files contribute legacy coordinates for events
          missing HPL records, joined via spatiotemporal matching on
          $[time, lat, lon, depth]$.
      \end{itemize}
...
self.EVENTS[OGS_C.NUMBER_P_AND_S_PICKS_STR] = self.EVENTS[
  ...
  $5 \times \text{--}10 \times $ storage volume reduction compared to
...
\end{verbatim}
  ...
  plotter classes built on Matplotlib and Cartopy, standardizing
  visual aesthetics across the project.
  ...
  \begin{enumerate}
    \item \textbf{Cartographic Mapping (\texttt{map\_plotter})}:
      Integrates Cartopy projections (PlateCarree and local
    ...
    \item \textbf{Cumulative Event Progression
      ...
      seismic shocks \eg{the $M_{\mathrm{L}}~4.6$ Socchieve mainshock}.
  ...
  \begin{equation}
    d_{\mathrm{horizontal}} = \frac{1}{1000} \cdot
  ...
  \begin{equation}
    d_{3\mathrm{D}} = \sqrt{d_{\mathrm{horizontal}}^2 +
      \left(\frac{|z_{\mathrm{hyp}} -
    z_{\mathrm{sta}}|}{1000}\right)^2} \quad [\unit{\kilo\meter}],
  ...
  To support high-throughput parallel execution on the CINECA
  ...
  storage architecture (Figure~\ref{fig:storage_architecture}):
  \begin{itemize}
    ...
    \item \textbf{Tier 1b: Day-Sharded Waveform Index (SQLite)}:
      Autonomous calendar-day Pyrocko Squirrel database shards stored
      ...
      RAM/tmpfs staging during inference.
  ...
  \begin{figure}[htbp]
    ...
    \begin{tikzpicture}[
      ...
      Trees}\\Zero-copy symlinks for HPC stage reuse};
    ...
    \caption{Hierarchical four-tier storage and data organization
      architecture deployed on the CINECA Leonardo HPC filesystem. The
      reader should notice the clear separation between read-only raw
      observation layers (Tiers 1 and 2), compressed columnar
      analytical catalogs (Tier 3), and zero-copy symbolic link
    execution structures (Tier 4).}
  ...
  \begin{enumerate}
    ...
    \item \textbf{Duplicate Rejection}: Detection and suppression of
      ...
      phase type, and arrival timestamps within $\Delta t < 0.01\,\mathrm{s}$.
