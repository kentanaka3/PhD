...
catalog acquired across Northeastern Italy during the three-month observation
window of the spring 2024 Socchieve earthquake crisis.
Section~\ref{sec:benchmark_datasets} presents an in-depth comparative review of
the four benchmark training corpora utilized to train and evaluate the
deep-learning phase pickers: \ac{INSTANCE} (Italy), \ac{STEAD} (global),
\ac{SCEDC} (Southern California), and the original author training
sets of PhaseNet and \ac{EQTransformer}. These collections span localized
regional to global tectonic environments and exhibit diverse signal-to-noise
ratios, noise textures, and labeling standards, establishing a testbed for
...
\begin{itemize}
  ...
  \item \textbf{Instrumental Network Aperture}: Waveform data were retrieved
    ...
    national, and international networks (Figure~\ref{fig:OGSStations}). The
  ...
  \item \textbf{Temporal Window and Observation Horizon}: The target
    observation window spans \textbf{93 continuous days}, from
    \placeholder{DATE RANGE} (00:00:00 UTC to 23:59:59 UTC). This
    \placeholder{NUM YEARS} horizon captures pre-seismic background
    microseismicity, the energetic mainshock and immediate aftershock sequence,
    and the subsequent rate decay of aftershocks over seasonal environmental
    transitions.
  \item \textbf{The March 27, 2024 Socchieve Earthquake}: The focal seismic
    ...
    the Carnic Prealps approximately $5\,\mathrm{km}$ south-southwest of
    Socchieve, Udine province (focal parameters: Latitude
      $46.3583^\circ\,\mathrm{N}$, Longitude $12.8080^\circ\,\mathrm{E}$,
      hypocentral depth $10.22\,\mathrm{km}$, local magnitude
  ...
  \item \textbf{Continuous Ingestion and Temporal Availability}: All continuous
    ...
    illustrated in Figure~\ref{fig:OGSAvailability}, demonstrating sustained
  ...
  \item \textbf{Sampling Rate Standardization}: All ingested
    ...
    acquisition frequencies ($50$, $100$, or $200\,\mathrm{Hz}$) to
    a standardized frequency of \textbf{100~Hz} ($\Delta t =
    0.01\,\mathrm{s}$) via polyphase anti-aliasing decimation,
...
\begin{figure}[htbp]
  ...
  official FDSN StationXML inventories from 18 participating networks.}
...
\begin{figure}[htbp]
  ...
  \IfFileExists{../MHPC/imgs/OGSCatalog/GaMMA0.1/img/.all_EventsMap.pdf}{%
    ...
    \IfFileExists{../MHPC/imgs/OGSCatalog/OGSEQTransformer_INSTANCE/img/.all_EventsMap.pdf}{%
      ...
      \MissingFigure{The event-map asset required for this figure is
        not present in the repository. A station-distribution map must
        not be substituted for an event-location map. Provide a
        versioned event-map artifact and its generating command before
      publication.}%
    }%
  }
...
\begin{figure}[htbp]
  ...
  \caption{Temporal station availability and daily trace acquisition
    metrics across the 93-day observation period (March 20 to June 20,
    2024). The reader should notice the sustained operational
    continuity across all networks, with minor transient drops
    resolved via automatic multi-client FDSN fallback. Data generated
  from continuous miniSEED audit logs parsed by \texttt{ogsutils.waveforms}.}
...
To establish a baseline reference for evaluating the machine-learning pipeline,
...
However, comparing machine-learning hypocenter locations derived
...
isolate the performance of the machine-learning picking and
association components, the \ac{OGS} reference events were independently
relocated using the identical \textbf{1D NonLinLoc}
(\cite{NonLinLoc}) velocity model and inversion parameterization
employed in our computational pipeline (Magrin, personal communication).
...
\begin{itemize}
  \item \textbf{\placeholder{NUM EVENTS} confirmed seismic events} strictly
    ...
    during the \placeholder{NUM YEARS} observation window.
  \item Local magnitudes ranging continuously from microseismic
    events ($M_{\mathrm{L}} = -0.2$) up to the $M_{\mathrm{L}}~4.6$
    Socchieve mainshock, with completeness threshold $M_{\mathrm{c}}
    \approx 1.2$.
  \item Focal depths predominantly confined between
    $5.0\,\mathrm{km}$ and $14.0\,\mathrm{km}$, reflecting the
    shallow seismogenic brittle layer of the Southern Alps.
  \item \textbf{\placeholder{NUM PICKS} manually verified phase picks},
    ...
    \textbf{\placeholder{NUM S PICKS} Secondary (S)} wave arrivals.
    Each pick includes analyst-assigned onset polarity, weight, and
    timing uncertainty.
\end{itemize}

This high-quality reference catalog provides the baseline for the
bipartite graph matching assessment (\ac{BPGMA}) developed in Chapter~5.
...
and \ac{EQTransformer}—rely on supervised training over massive
...
framework (\cite{SeisBench}). Each dataset represents a distinct
...
The \acf{INSTANCE} dataset (\cite{INSTANCE}) is a comprehensive
open-access collection of labeled seismic waveforms recorded across
the Italian peninsula between 2005 and 2020 by the National Seismic
Network (RSN), operated by \ac{INGV}, and partner regional networks
including \ac{OGS}.

Unlike global machine-learning corpora curated primarily for high
signal-to-noise ratios, INSTANCE was specifically designed to
preserve the realistic operational conditions of active regional
monitoring networks:
\begin{itemize}
  \item \textbf{Operational Noise Realism}: INSTANCE incorporates
  ...
  \item \textbf{Volume and Balance}: INSTANCE comprises nearly
    \textbf{1.3 million three-component waveform traces}
    ($1,291,537$), including $1,159,249$ labeled P-wave arrivals,
    $713,883$ labeled S-wave arrivals, and a dedicated negative
    class of $132,288$ pure noise windows extracted during periods
    without seismic activity.
  \item \textbf{Geographical Proximity}: Because INSTANCE waveforms
    originate from Italy, the training wavefields reflect crustal
    attenuation, complex fault structures, and station vault
    installations physically similar to those of our Northeastern
    Italy study area.
\end{itemize}
...
\begin{figure}[htbp]
  ...
  \caption{Geographical station distribution and event density of
    the INSTANCE benchmark dataset across Italy. The reader should
    notice the extensive coverage along the Apenninic chain and the
    Southern Alpine arc, capturing realistic Mediterranean crustal
    paths and anthropogenic noise conditions. Image reproduced from
  \textcite{SeisBench}.}
...
The \acf{STEAD} dataset (\cite{STEAD}) is a globally distributed
benchmark corpus curated by researchers at Stanford University to
facilitate artificial intelligence research in seismology. Sourced
from seismic stations worldwide, STEAD incorporates waveforms from
...
\begin{itemize}
  ...
  \item \textbf{Extensive Labeling}: STEAD contains over
    \textbf{1.26 million three-component traces} ($1,265,657$), with
    $1,030,231$ P-wave picks and $1,030,231$ S-wave picks across
    $441,705$ distinct earthquakes.
  \item \textbf{Dedicated Noise Partition}: The dataset includes
    $235,426$ continuous noise recordings sampled across global
  ...
  \item \textbf{Global Transferability}: By encompassing a wide
    ...
    $350\,\mathrm{km}$), STEAD is engineered to prevent models from
...
\begin{figure}[htbp]
  ...
  \caption{Worldwide distribution of seismological stations
    contributing waveform recordings to the global STEAD benchmark
    dataset. The reader should notice the broad geographic and
    tectonic representation across diverse orogenic belts and plate
  boundaries. Image reproduced from \textcite{SeisBench}.}
...
The \acf{SCEDC} dataset (\cite{SCEDC}) provides an unprecedented
volume of seismic recordings curated from the Southern California
Seismic Network (SCSN), spanning decades of continuous monitoring
across the San Andreas fault system.

SCEDC is characterized by:
\begin{itemize}
  \item \textbf{Massive Data Volume}: Sourced through SeisBench, the
    SCEDC corpus contains \textbf{8.11 million three-component
    traces} ($8,111,060$), incorporating $7,571,970$ P-wave picks
    and $4,364,155$ S-wave picks across $378,528$ local earthquakes.
  \item \textbf{Dense Local Network Geometry}: The SCSN features an
    exceptionally high spatial sensor density, capturing
    high-frequency wavefield details over purely local propagation
    paths ($\Delta \le 150\,\mathrm{km}$).
  \item \textbf{Catalog Completeness}: Decades of continuous analyst
    ...
    the SCEDC catalog complete to very low magnitude thresholds
...
\begin{figure}[htbp]
  ...
  \caption{Spatial station distribution of the Southern California
    Seismic Network (SCSN) represented in the SCEDC dataset. The
  ...
  reproduced from \textcite{SeisBench}.}
...
evaluate the models trained on the original corpora established by
the respective model architects:
\begin{itemize}
  \item \textbf{Original PhaseNet Corpus} (\cite{PhaseNet}): Curated
    by the Northern California Seismic Network (NCSN), this dataset
  ...
  \item \textbf{Original EQTransformer Corpus}
    (\cite{EQTransformer}): Curated as a specialized subset of the
    STEAD repository, comprising approximately $1.2$ million
    three-component traces selected to optimize multi-task
    transformer training for simultaneous earthquake detection, P
    picking, and S picking across 60-second waveform windows.
\end{itemize}
...
$100\,\mathrm{Hz}$, either natively or through standardized
...
\begin{table}[htbp]
  ...
  \caption{Systematic comparison of seismological benchmark training
    ...
    INSTANCE. Traces, noise windows, and pick counts are derived from
    SeisBench metadata schemas; distance bounds classify local
    ($\Delta \le 150\,\mathrm{km}$) versus regional ($150 < \Delta \le
  600\,\mathrm{km}$) propagation paths.}
  \label{tab:datasets}
  \begin{threeparttable}
    \begin{tabularx}{\linewidth}{l r r r r c l}
      ...
      SCEDC    & \num{8111060} & \num{7571970} &
      \num{4364155} & \num{378528} & 0\tnote{b} & S. California (L) \\
      PhaseNet\tnote{c} & \num{779514} &
      \multicolumn{2}{c}{--- \tnote{d} ---} & \num{234117} &
      0\tnote{b} & N. California (L) \\
      EQTransformer\tnote{c} & \num{1200000} & \num{1000000}
    ...
    \item[a] Epicentral distance classification: Local
      ($\mathrm{L}$) denotes $\Delta \le 150\,\mathrm{km}$; Regional
      ($\mathrm{R}$) denotes $150 < \Delta \le 600\,\mathrm{km}$.
    \item[b] Dedicated pure noise trace partitions were not
      independently generated in the SCEDC and original PhaseNet
      repositories; ambient noise representations are extracted from
      pre-P arrival waveform windows.
    \item[c] Original PhaseNet and EQT datasets as described in their
      respective publications.
    \item[d] In the original PhaseNet publication (\cite{PhaseNet}),
...
(such as the 8.1 million traces of SCEDC in Southern California)
outperform a network trained on a smaller but regionally congruent
dataset (such as the 1.3 million traces of INSTANCE in Italy)?
...
\begin{enumerate}
  ...
  \item \textbf{Anthropogenic and Environmental Noise Spectra}:
    ...
    (characterized by 50~Hz electrical harmonics, narrow road
...
By systematically evaluating PhaseNet and EQTransformer models
trained across INSTANCE, STEAD, SCEDC, and the original corpora on
the identical three-month SMINO continuous dataset, this study
