...
% Source-of-truth: OGS/config/, doc/MHPC/chapters/appendix.tex,
...
velocity-model path, and GaMMA parameters. A publication-ready
  ...
  \begin{table}[htbp]
    ...
    \begin{threeparttable}
      \begin{tabularx}{\linewidth}{l l l c X}
        ...
        \texttt{starttime} & ISO Date & \texttt{'2024-03-20'} &
        [Observed] & Beginning timestamp of continuous observation
        horizon (00:00:00 UTC) \\
        \texttt{endtime} & ISO Date & \texttt{'2024-06-20'} &
  ...
  \begin{table}[htbp]
    ...
    \begin{threeparttable}
      ...
      \item What the reader should notice: Large batch sizing
        (\num{16384}) maximizes GPU tensor core occupancy, achieving
  ...
  \begin{table}[htbp]
    ...
    \begin{threeparttable}
      \begin{tabularx}{\linewidth}{l l l c X}
        ...
        ($0\text{--}40\,\unit{\kilo\meter}$) \\
        \texttt{vel.p, vel.s} & Float [$\unit{\kilo\meter/\second}$] &
        $5.85, 3.28$ & [Analyst-Normalized] & Apparent body-wave
        ...
        \texttt{max\_sigma11} & Float & $1.0$ & [Model Hypothesis] &
      ...
      \item Evidence Anchor: The current source is
        \path{OGS/config/group_modules/associator/ogsgamma.yaml};
  ...
  \begin{table}[htbp]
    ...
    \begin{threeparttable}
      \begin{tabularx}{\linewidth}{l l l c X}
        ...
        \texttt{total\_picks} & Integer & $6$ & Minimum associated
        picks required
        for event survival into location \\
        \texttt{p\_and\_s\_picks} & Integer & $3$ & Minimum stations recording
      ...
      \item Target \& Scope: Quality-control filter gating associated
        event hypotheses prior to NonLinLoc inversion.
...
Table~\ref{tab:app_nll_dictionary} specifies the NonLinLoc inversion cards.
  ...
  \begin{table}[htbp]
    ...
    \begin{threeparttable}
      \begin{tabularx}{\linewidth}{l l l c X}
        ...
        \texttt{LOCGAU} & Error Spec & $0.15\,\unit{\second},
        1.0\,\unit{\second}$ & [Analyst-Normalized] & Correlation
      ...
      \begin{tablenotes}[flushleft]
        \footnotesize
      \item Target \& Scope: NonLinLoc probabilistic relocation cards
        under the 1D crustal model.
      \item What the reader should notice: The EDT formulation
        decouples origin time from spatial coordinates, rendering
        hypocentral convergence robust against outlier picks.
      \item Evidence Anchor: The current source is
        \path{OGS/config/group_modules/nonlinloc/ogsnonlinloc1d.yaml};
        values in this table describe the historical example and
        require run-level confirmation.
      \end{tablenotes}
    \end{threeparttable}
  ...
  \begin{table}[htbp]
    ...
    \begin{threeparttable}
      \begin{tabularx}{\linewidth}{l l l c X}
        ...
        \texttt{n\_workers} & Integer & $1$ & [System Architecture] &
        Worker process count per MPI task allocation \\
      ...
      \item Evidence Anchor: Implemented in
        \path{OGS/src/ogsmagnitude.py} and \path{OGS/config/config.yaml}.
      \end{tablenotes}
    \end{threeparttable}
  \end{table}
\end{landscape}
