...
FDSN registry and the access date used for the submitted thesis.
  ...
  \begin{table}[htbp]
    ...
    \begin{threeparttable}
      \begin{tabularx}{\linewidth}{l l l c X}
        ...
        2005--pres. & FDSN temporary / project archive \\
        \textbf{IT} & Italian Strong Motion Network (RAN) & Civil
        Protection Department & 1972--pres. &
        \url{https://doi.org/10.7914/SN/IT} \\
        \textbf{IV} & Rete Sismica Nazionale (RSN) & INGV &
        2005--pres. & \url{https://doi.org/10.13127/sd/x0fxnh7qfy} \\
        \textbf{MN} & Mediterranean Network (MedNet) & INGV / Partners
        ...
        \textbf{RF} & FVG Accelerometric Network (RAF) & University of
        Trieste & 1993--pres. & \url{https://doi.org/10.7914/SN/RF} \\
        \textbf{S}  & Seismo-at-School Program & SED at ETH Z\"urich &
        ...
        Slovenian Env.\ Agency (ARSO) & 1990--pres. &
        ...
        \textbf{ZO} & Garda Multiparametric Network (PDnet) & INGV &
      ...
      \item Evidence Anchor: Formally registered FDSN network codes and
...
\begin{enumerate}[label={[\textbf{\arabic*}]},leftmargin=*]
  ...
  \item \textbf{[NI] \acf{OGS} and University of Trieste (2002)}.
...
\begin{itemize}
  \item \textbf{INSTANCE (Italy)}: Michelini, A., Cianetti, S.,
  ...
  \item \textbf{STEAD (Global)}: Mousavi, S.~M., Sheng, Y., Zhu, W.,
  ...
  \item \textbf{SCEDC (Southern California)}: Southern California
  ...
  \item \textbf{PhaseNet Original Weights}: Zhu, W., \& Beroza,
  ...
  \item \textbf{Earthquake Transformer Original Weights}: Mousavi,
...
\begin{itemize}
  \item \textbf{\ac{OGS} Pipeline Framework}: The Python source code
    (\texttt{OGS/src/}) implementing multi-format catalog parsing,
    FDSN downloading, bipartite graph matching (\ac{BPGMA}), local
    magnitude calculation, and plotting is maintained in the
    institutional repository:
    \url{https://github.com/kentanaka3/PhD}.
  \item \textbf{CINECA Leonardo HPC Orchestration}: All Slurm
    submission scripts, Hydra configuration templates, Makefile
    dependency graphs, and environment activation wrappers
    (\texttt{OGS/utils/Leonardo/}) are preserved within the
    repository under release tag \texttt{v1.0-UNITS-PhD}.
  \item \textbf{SeisBench Platform}: Pre-trained models and dataset
    interfaces were executed using SeisBench version 0.3.4 (\cite{SeisBench}).
  \item \textbf{Associator Codes}:
    \begin{itemize}
      \item \textbf{GaMMA}: Available at
        \url{https://github.com/AI4EPS/GaMMA} (\cite{GaMMA}).
      \item \textbf{PyOcto}: Available at
        \url{https://github.com/austin-bell/pyocto} (\cite{PyOcto}).
    \end{itemize}
  \item \textbf{Relocation Engine}: The NonLinLoc software suite
    (version 7.00) developed by Anthony Lomax is accessible at
    \url{https://alomax.free.fr/nlloc/} (\cite{NonLinLoc}).
\end{itemize}
