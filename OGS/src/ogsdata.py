...
def _fast_update_squirrel_db(sq, squirrel_add_paths: list[pathlike]) -> None:
  ...
  to_remove = [
      ...
      for f in db_files
      if (
          f not in target_files and
          _is_under_resolved_roots(Path(f), resolved_roots)
      )
  ]
  ...
  if to_remove:
    logger.info(
        f"Fast-removing {len(to_remove)} deleted files from Squirrel DB"
    )
    try:
      sq.remove(to_remove)
    except Exception as e:
...
def _index_day_worker(
  ...
  try:
    ...
    import shutil
    try:
      squirrel_cache = db_dir / ".squirrel"
      if squirrel_cache.is_dir():
        shutil.rmtree(squirrel_cache)
    except Exception:
      pass
    raise RuntimeError(f"Error indexing day {day_iso}: {e}") from e
...
def _get_cached_ogs_day_dirs(
  ...
  signature = _ogs_day_dir_cache_signature(
...
def _select_squirrel_add_paths(
  ...
  for original_path in paths:
    ...
    day_dirs = _get_cached_ogs_day_dirs(
...
class OGSSquirrelDataSource(BaseSquirrelDataSource):
  ...
  def __init__(
    ...
    if self.day_envs:
      ...
      logger.info(
          ...
          f"{existing_count}, planned shards: "
          f"{len(self.day_envs) - existing_count}, "
          f"days: {len(self.day_envs)}"
      )
    else:
      logger.warning(
          "No day-sharded databases found matching "
          f"{env_path.name}/YYYY/MM/DD. "
      ...
      if not (env_path / ".squirrel").is_dir():
        ...
        logger.warning(
            "Base Squirrel environment not found. "
            "Initialising new environment."
        )
    ...
    if self.check:
      if self.day_envs:
        ...
        logger.info(
            "Adding/checking paths in monolithic Squirrel (fast set-difference updates)")
      ...
      logger.debug(
          "Skipping writable master setup (using pre-indexed read-only sharded/base DBs)")
  ...
  def get_sq(self, day: datetime.date):
    ...
    If day-sharded mode is active and the day has a shard, that shard is used.
    Otherwise the monolithic base Squirrel instance is returned. When
  ...
  def base_sq(self):
    ...
    if self._base_sq is None:
      ...
      self._base_sq = squirrel_cls(
          env=self._tmp_squirrel_path, persistent=self.persistent
      )
    return self._base_sq
  ...
  def _query_days(self) -> list[datetime.date]:
    ...
    """
    return (sorted(self.day_envs.keys()) if self.day_envs
            else list(self.active_days))
  ...
  def _get_all_station_codes(self) -> list:
    ...
    The result is used by ``get_group`` to try station/channel combinations in
    priority order. If sharded metadata lookup fails or returns empty, the
    method falls back to the monolithic base database.
    """
    codes_set = set()
    if self.day_envs:
      for d in self._query_days():
        try:
          ...
          logger.warning(
              f"Failed to get station codes for day {d.isoformat()}: {e}"
          )
  ...
  def _check_group(self, group: str) -> bool:
    ...
    for d in self._group_days(t0, t1):
      ...
      try:
        ...
        logger.warning(
            f"Error checking group waveforms for {group} on "
            f"day {d.isoformat()}: {e}"
        )
        continue
      if len(nuts) > 0:
  ...
  def get_group(self, group: str) -> DataInterface:
    ...
    For each station code, channels are tried in ``channel_priorities`` order.
    The first channel pattern that returns waveform data is yielded as an
    ObsPy stream, then the method moves to the next station. When a group
    spans more than one day, traces from every intersecting day shard are
    concatenated before being yielded.
    """
    t0, t1 = self._parse_group_times(group)
  ...
  def pyrocko_traces_to_obspy_stream(traces: list) -> obspy.Stream:
    ...
    """
    return obspy.Stream(
        [obspy_compat.to_obspy_trace(trace) for trace in traces]
    )
  ...
  def citations(self) -> list[str]:
    ...
    return [
        ...
        "  author={Heimann, Sebastian and Kriegerowski, Marius and Isken, Marius and Cesca, Simone and Daout, Simon and Grigoli, Francesco and Juretzek, Carina and Megies, Tobias and Nooshiri, Nima and Steinberg, Andreas and others},\n"
        "  year={2017},\n"
