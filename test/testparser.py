...
class TestParser(unittest.TestCase):
  ...
  def test_event_parser_dat(self):
    ...
    _, data = event_parser_dat(args.file[0], *args.dates, args.station)
  ...
  def test_event_parser_hpl(self):
    args = ini.parse_arguments()
    source, detect = event_parser_hpl(args.file[0], *args.dates, args.station)
  ...
  def test_event_parser_qml(self):
    args = ini.parse_arguments()
    source, detect = event_parser_qml(args.file[0], *args.dates, args.station)
    source = source.values.tolist()
    EXPECTED = [
  ...
  def test_event_parser_folder(self):
    args = ini.parse_arguments()
    source, detect = event_parser(args.file[0], *args.dates, args.station)
    source = source.values.tolist()
    EXPECTED = [
      [898, UTCDateTime(2023, 6, 1, 9, 27, 56, 980000), '45-56.52', '12-22.63',
