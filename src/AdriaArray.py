...
MODEL_WEIGHTS_DICT = {
  EQTRANSFORMER_STR : {
    ...
    WEIGHT_STR : (ETHZ_STR, GEOFON_STR, INSTANCE_STR, ORIGINAL_STR, SCEDC_STR,
                  STEAD_STR)
  },
  GPD_STR           : {
    CLASS_STR : sbm.GPD(),
    WEIGHT_STR : (ETHZ_STR, GEOFON_STR, NEIC_STR, ORIGINAL_STR, SCEDC_STR,
                  STEAD_STR)
  },
  PHASENET_STR      : {
    CLASS_STR : sbm.PhaseNet(),
    WEIGHT_STR : (ETHZ_STR, GEOFON_STR, INSTANCE_STR, NEIC_STR, SCEDC_STR,
                  STEAD_STR)
  }
}
...
MSEED_FMT = "{NETWORK}.{STATION}..{CHANNEL}__{BEGDT}T{BEGTM}Z__{ENDDT}T"\
            "{ENDTM}Z" + MSEED_EXT
...
parser.add_argument('-G', "--groups", default=[BEG_DATE_STR], nargs='+',
                    required=False, metavar="",
                    help="Analize the data based on a specified categorically "
                         "ordered list")
parser.add_argument('-R', "--range", nargs=2, required=False, type=str,
                    metavar="BEGDT ENDDT", default=(20230601, 20230731),
...
def main(args):
  ...
  MODEL = args.models
  PRETRAINED_WEIGHT = args.weights
  GROUPS = args.groups
  groupMap = {
    NETWORK_STR : "*",
    STATION_STR : "*",
    CHANNEL_STR : "*",
    BEG_DATE_STR : args.range[0],
    BEG_TIME_STR : "000000",
    END_DATE_STR : "*",
    END_TIME_STR : "*",
  }
  for x, y in list(itertools.product(MODEL, PRETRAINED_WEIGHT)):
    if y in MODEL_WEIGHTS_DICT[x][WEIGHT_STR]:
      ...
      for group in WAVEFORMS_DATA.groupby(GROUPS):
        ...
        # Clean the stream
        for trc in stream:
          TRC_FILE = os.path.join(prc_data, trc.id + "_" + d + MSEED_EXT)
